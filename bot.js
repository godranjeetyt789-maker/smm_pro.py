...
const DEFAULTS = {
  ...
  MIN_RECHARGE : 10,
  // Payment Gateway
  GMAIL_USER   : process.env.GMAIL_USER   || 'godranjeetyt879@gmail.com',
  GMAIL_PASS   : process.env.GMAIL_PASS   || 'urqh icql vomd hohu',
...
// ──────────────────────────────────────────────
//  SMART LINK DETECTION & VALIDATION
//  Detects what kind of link a service needs
//  based on service name + category keywords.
// ──────────────────────────────────────────────
function detectLinkType(svcName, category) {
  const n = (svcName||'').toLowerCase();
  const c = (category||'').toLowerCase();

  const isTelegram = n.includes('telegram') || c.includes('telegram');
  const isYoutube  = n.includes('youtube')  || c.includes('youtube');
  const isInsta    = n.includes('instagram')|| c.includes('instagram');
  const isTiktok   = n.includes('tiktok')   || c.includes('tiktok');
  const isFacebook = n.includes('facebook') || c.includes('facebook');
  const isTwitter  = n.includes('twitter')  || n.includes(' x ') || c.includes('twitter') || c.includes(' x ');
  const isSpotify  = n.includes('spotify')  || c.includes('spotify');
  const isLinkedin = n.includes('linkedin') || c.includes('linkedin');

  if (isTelegram) {
    // Post/message reactions, views, comments → need post URL with number
    const isPost = n.includes('post') || n.includes('message') || n.includes('react') || n.includes('reaction') || n.includes('comment');
    // Views alone could be post or channel — if it says "post views" → post
    const isPostView = (n.includes('post') && n.includes('view')) || n.includes('post view');
    // Members/subscribers — need PRIVATE invite link for groups
    const isMember = n.includes('member') || n.includes('join') || (n.includes('group') && !n.includes('channel'));
    // Channel views (without "post") or just plain "views" → post link
    const isView = n.includes('view') && !n.includes('channel');

    if (isPost || isPostView || (isView && !isMember)) return 'TELEGRAM_POST';
    if (isMember) return 'TELEGRAM_INVITE';
    // Channel subscribers/followers → public channel link
    return 'TELEGRAM_PUBLIC';
  }

  if (isYoutube) {
    const isVideo = n.includes('view') || n.includes('like') || n.includes('comment') || n.includes('share') || n.includes('short');
    return isVideo ? 'YOUTUBE_VIDEO' : 'YOUTUBE_CHANNEL';
  }

  if (isInsta) {
    const isPost = n.includes('like') || n.includes('view') || n.includes('comment') || n.includes('save') || n.includes('reel') || n.includes('impression');
    return isPost ? 'INSTAGRAM_POST' : 'INSTAGRAM_PROFILE';
  }

  if (isTiktok) {
    const isVideo = n.includes('view') || n.includes('like') || n.includes('share') || n.includes('comment') || n.includes('play');
    return isVideo ? 'TIKTOK_VIDEO' : 'TIKTOK_PROFILE';
  }
...
// Returns emoji + title + full instruction text for the link prompt
function getLinkInfo(linkType) {
  const m = {
    TELEGRAM_INVITE: {
      emoji: '🔐',
      title: 'Private Group Invite Link',
      msg: `🔗 <b>Please send your link:</b>`,
      example: 'https://t.me/+AbCdEfGhIjKlMn',
    },
    TELEGRAM_POST: {
      emoji: '📱',
      title: 'Telegram Post Link',
      msg: `🔗 <b>Apna link bhejein:</b>`,
      example: 'https://t.me/yourchannel/42',
    },
    TELEGRAM_PUBLIC:  { emoji:'📢', title:'Telegram Channel/Group Link', msg:`🔗 <b>Please send your link:</b>`, example:'https://t.me/yourchannel' },
    YOUTUBE_VIDEO:    { emoji:'▶️', title:'YouTube Video Link',          msg:`🔗 <b>Please send your link:</b>`, example:'https://youtu.be/xxxxxxx' },
    YOUTUBE_CHANNEL:  { emoji:'📺', title:'YouTube Channel Link',        msg:`🔗 <b>Please send your link:</b>`, example:'https://youtube.com/@yourchannel' },
    INSTAGRAM_POST:   { emoji:'📸', title:'Instagram Post / Reel Link',  msg:`🔗 <b>Please send your link:</b>`, example:'https://instagram.com/p/Cxxxxxxxxxx/' },
    INSTAGRAM_PROFILE:{ emoji:'📸', title:'Instagram Profile Link',      msg:`🔗 <b>Please send your link:</b>`, example:'https://instagram.com/yourusername' },
    TIKTOK_VIDEO:     { emoji:'🎵', title:'TikTok Video Link',           msg:`🔗 <b>Please send your link:</b>`, example:'https://tiktok.com/@user/video/1234567890' },
    TIKTOK_PROFILE:   { emoji:'🎵', title:'TikTok Profile Link',         msg:`🔗 <b>Please send your link:</b>`, example:'https://tiktok.com/@yourusername' },
    FACEBOOK:         { emoji:'👍', title:'Facebook Link',               msg:`🔗 <b>Please send your link:</b>`, example:'https://facebook.com/yourpage' },
    TWITTER:          { emoji:'🐦', title:'Twitter/X Link',              msg:`🔗 <b>Please send your link:</b>`, example:'https://x.com/yourusername' },
    SPOTIFY:          { emoji:'🎧', title:'Spotify Link',                msg:`🔗 <b>Please send your link:</b>`, example:'https://open.spotify.com/track/xxxxxxx' },
    LINKEDIN:         { emoji:'💼', title:'LinkedIn Link',               msg:`🔗 <b>Please send your link:</b>`, example:'https://linkedin.com/in/yourname' },
    DEFAULT:          { emoji:'🔗', title:'Link',                        msg:`🔗 <b>Please send your link:</b>`, example:'https://example.com/yourprofile' },
  };
  return m[linkType] || m.DEFAULT;
}

// Validates the link based on detected type; returns {ok, link, msg}
function validateLink(text, linkType) {
  const t = (text||'').trim();
  const lower = t.toLowerCase();

  // Handle @username shorthand
  if (t.startsWith('@')) {
    if (['TELEGRAM_PUBLIC','INSTAGRAM_PROFILE','TWITTER'].includes(linkType)) {
      return { ok:true, link:t };
    }
    const info = getLinkInfo(linkType);
    return { ok:false, msg:`❌ <b>Galat format!</b>\n\nIs service ke liye <code>@username</code> kaam nahi karega.\n\n✅ Sahi example:\n<code>${info.example}</code>` };
  }

  if (!isURL(t)) {
    const info = getLinkInfo(linkType);
    return { ok:false, msg:`❌ <b>Invalid link!</b>\n\n<code>https://</code> se start hona chahiye.\n\n✅ Example:\n<code>${info.example}</code>\n\nDobara bhejein:` };
  }

  switch (linkType) {
    case 'TELEGRAM_INVITE': {
      const valid = lower.includes('t.me/+') || lower.includes('t.me/joinchat/');
      if (!valid) return { ok:false, msg:
        `❌ <b>Galat link!</b>\n\n` +
        `Is service ke liye <b>Private Invite Link</b> chahiye.\n\n` +
        `✅ <b>Sahi:</b>\n<code>https://t.me/+xxxxxxxxxx</code>\n<code>https://t.me/joinchat/xxxxxxxxxx</code>\n\n` +
        `❌ <b>Aapne jo bheja (galat):</b>\n<code>${safeSlice(t,50)}</code>\nYeh public link hai, private members ke liye kaam nahi karega.\n\n` +
        `💡 Group → Settings → Invite Links → Copy karein.\n\nSahi link bhejein:`
      };
      return { ok:true, link:t };
    }
    case 'TELEGRAM_POST': {
      const postNum = /t\.me\/[^/+?]+\/(\d+)/.test(lower);
      if (!postNum) return { ok:false, msg:
        `❌ <b>Galat link!</b>\n\n` +
        `Yeh Telegram post link nahi hai. Post number hona chahiye.\n\n` +
        `✅ <b>Sahi:</b>\n<code>https://t.me/channelname/123</code>\n\n` +
        `❌ <b>Aapne jo bheja:</b>\n<code>${safeSlice(t,50)}</code>\n\n` +
        `💡 Post kholein → ⋮ → Copy Link\n\nSahi link bhejein:`
      };
      return { ok:true, link:t };
    }
    case 'TELEGRAM_PUBLIC': {
      if (!lower.includes('t.me/')) return { ok:false, msg:
        `❌ <b>Galat link!</b>\n\n✅ <b>Sahi:</b>\n<code>https://t.me/yourchannelname</code>\nya\n<code>@yourchannelname</code>\n\nSahi link bhejein:`
      };
      // Warn if they sent an invite link for a public-channel service
      if (lower.includes('t.me/+') || lower.includes('t.me/joinchat/')) return { ok:false, msg:
        `❌ <b>Galat link!</b>\n\nYeh private invite link hai, lekin is service ke liye public channel username chahiye.\n\n✅ <b>Sahi:</b>\n<code>https://t.me/yourchannelname</code>\n\nSahi link bhejein:`
      };
      return { ok:true, link:t };
    }
    case 'YOUTUBE_VIDEO': {
      const isVid = lower.includes('youtube.com/watch') || lower.includes('youtu.be/') || lower.includes('youtube.com/shorts/');
      if (!isVid) return { ok:false, msg:
        `❌ <b>Galat link!</b>\n\nVideo link chahiye, channel link nahi.\n\n✅ <b>Sahi:</b>\n<code>https://youtube.com/watch?v=xxxxxxx</code>\n<code>https://youtu.be/xxxxxxx</code>\n\nSahi link bhejein:`
      };
      return { ok:true, link:t };
    }
    case 'YOUTUBE_CHANNEL': {
      const isCh = lower.includes('youtube.com/channel/') || lower.includes('youtube.com/@') || lower.includes('youtube.com/c/') || lower.includes('youtube.com/user/');
      if (!isCh) return { ok:false, msg:
        `❌ <b>Galat link!</b>\n\nChannel link chahiye.\n\n✅ <b>Sahi:</b>\n<code>https://youtube.com/@channelname</code>\n<code>https://youtube.com/channel/UCxxxxxxx</code>\n\nSahi link bhejein:`
      };
      return { ok:true, link:t };
    }
    case 'INSTAGRAM_POST': {
      // Accept any instagram.com link that points to actual content
      // (post, reel, tv, LIVE, stories, highlights) — not just a bare profile URL.
      const isProfileOnly = /^https?:\/\/(www\.)?instagram\.com\/[a-zA-Z0-9_.]+\/?(\?.*)?$/i.test(t);
      const isInstaLink = lower.includes('instagram.com/');
      const isPost = isInstaLink && !isProfileOnly;
      if (!isPost) return { ok:false, msg:
        `❌ <b>Galat link!</b>\n\nPost/Reel/Live ka link chahiye, sirf profile nahi.\n\n✅ <b>Sahi:</b>\n<code>https://instagram.com/p/xxxxxxxxxx/</code>\n<code>https://instagram.com/reel/xxxxxxxxxx/</code>\n<code>https://instagram.com/username/live/xxxxxxxxxx/</code>\n\nSahi link bhejein:`
      };
      return { ok:true, link:t };
    }
    case 'INSTAGRAM_PROFILE': {
      if (!lower.includes('instagram.com/')) return { ok:false, msg:
        `❌ <b>Galat link!</b>\n\n✅ <b>Sahi:</b>\n<code>https://instagram.com/yourusername</code>\nya\n<code>@yourusername</code>\n\nSahi link bhejein:`
      };
      return { ok:true, link:t };
    }
    case 'TIKTOK_VIDEO': {
      const isVid = lower.includes('tiktok.com/') && (lower.includes('/video/') || lower.includes('vm.tiktok') || lower.includes('vt.tiktok'));
      if (!isVid) return { ok:false, msg:
...
function displayOrderId(o) {
  // Show the REAL panel order ID to the user for API orders (matches the
  // website), and the internal bot ID only for custom (non-API) orders.
  return (!o.isCustom && o.apiOrderId) ? o.apiOrderId : o.id;
}
...
// ──────────────────────────────────────────────
//  BUILT-IN GMAIL PAYMENT VERIFY — No external gateway
// ──────────────────────────────────────────────

// QR — tumhara apna UPI ID, ref_id transaction note mein
function gatewayQrImageUrl(amount, orderId) {
  const C = cfg(loadDB());
  const upiStr = `upi://pay?pa=${C.upiId}&pn=${encodeURIComponent(C.upiName)}&am=${amount}&tn=${encodeURIComponent(orderId)}&cu=INR`;
...
function genRefId() {
  ...
  return 'RJT' + ts + crypto.randomBytes(4).toString('hex').toUpperCase();
}

// Gmail IMAP verify — FIXED: refId-first matching to prevent duplicate approvals
async function checkGatewayOrder(refId, amount) {
  return new Promise((resolve) => {
    const gmailUser = DEFAULTS.GMAIL_USER;
    const gmailPass = DEFAULTS.GMAIL_PASS;
    if (!gmailUser || !gmailPass) {
      return resolve({ ok: false, isPaid: false, error: 'Gmail credentials missing' });
    }

    // ── DUPLICATE PREVENTION: Check if this refId already approved in DB ──
    const db = loadDB();
    const alreadyApproved = Object.values(db.payments).find(
      p => (p.gwOrderId === refId || p.id === refId) && p.status === 'approved'
    );
    if (alreadyApproved) {
      console.log(`[Gmail] RefId ${refId} already approved in DB — skipping email check`);
      return resolve({ ok: true, isPaid: true, status: 'already_approved' });
    }
    ...
    imap.once('error', (e) => {
      console.error('[Gmail] IMAP error:', e.message);
      resolve({ ok: false, isPaid: false, error: e.message });
    });

    imap.once('ready', () => {
      imap.openBox('INBOX', true, (err) => {
        if (err) { imap.end(); return resolve({ ok:false, isPaid:false, error:err.message }); }

        // Aaj ki date se search karo (IMAP SINCE sirf date support karta hai)
        const sinceStr = new Date().toLocaleDateString('en-GB', {
          day:'2-digit', month:'short', year:'numeric'
        }).replace(/ /g,'-'); // e.g. "26-Jul-2026"

        imap.search(['ALL', ['SINCE', sinceStr]], (err2, results) => {
          if (err2 || !results || results.length === 0) {
            imap.end();
            return resolve({ ok:true, isPaid:false, status:'pending', error:'No emails today' });
          }

          // Last 100 emails fetch karo (aaj ke)
          const toFetch = results.slice(-100);
          const f = imap.fetch(toFetch, { bodies: '' });
          let found = false;
          let foundByRef = false; // Track if matched by refId (stricter)

          // Amount ke saare possible formats
          const amt = parseFloat(amount);
          const amtInt = Math.floor(amt);
          const amtPatterns = [
            `₹${amt}`, `₹${amtInt}`, `₹ ${amt}`, `₹ ${amtInt}`,
            `rs.${amt}`, `rs ${amt}`, `rs.${amtInt}`, `rs ${amtInt}`,
            `inr ${amt}`, `inr${amt}`, `inr ${amtInt}`, `inr${amtInt}`,
            ` ${amt} `, ` ${amtInt} `,
            `${amt}.00`, `${amtInt}.00`,
            `amount ${amt}`, `amount ${amtInt}`,
            `rupees ${amtInt}`, `rs/-${amtInt}`,
          ].map(s => s.toLowerCase());

          // Payment success keywords (PhonePe, GPay, Paytm, bank sms style)
          const successKw = [
            'debited','credited','received','paid','success','successful',
            'payment done','payment received','payment successful',
            'upi','transferred','sent','transaction successful',
            'money sent','amount debited','amount credited',
          ];

          // UPI transaction note patterns — refId is embedded in these
          // UPI note mein transaction note = refId hota hai
          const refPatterns = refId ? [
            refId.toLowerCase(),
            refId.toLowerCase().replace('rjt',''), // without prefix
          ] : [];
          ...
          f.on('message', (msg) => {
            ...
            msg.once('end', async () => {
              ...
              try {
                ...
                const amtOk = amtPatterns.some(p => full.includes(p));
                const kwOk  = successKw.some(k => full.includes(k));
                // ── STRICT: RefId match — UPI transaction note mein refId hona chahiye ──
                // RJT2026... format ka refId UPI note mein aata hai jab user ne QR scan kiya
                const refOk = refPatterns.length > 0 && refPatterns.some(r => full.includes(r));

                console.log(`[Gmail] subj="${parsed.subject?.slice(0,60)}" | amt=${amtOk} kw=${kwOk} ref=${refOk} | refId=${refId}`);

                // ── PRIORITY 1: RefId match (most reliable) ──
                // Agar refId email mein hai — direct approve (amount bhi hona chahiye)
                if (refOk && amtOk) {
                  found = true;
                  foundByRef = true;
                  console.log(`[Gmail] ✅ MATCHED BY REF ID: ${refId}`);
                }
                // ── PRIORITY 2: Amount + keyword (fallback, less strict) ──
                // ONLY for small amounts (≤ ₹10) where refId might not appear in email
                // For larger amounts, we require refId to avoid false matches
                else if (amtOk && kwOk && amt <= 10) {
                  // Small amount — allow amount+keyword match as fallback
                  // BUT only if no other payment with same amount is already approved today
                  const dbCheck = loadDB();
                  const sameAmtApproved = Object.values(dbCheck.payments).find(p =>
                    p.status === 'approved' &&
                    parseFloat(p.amount) === amt &&
                    p.approvedAt && (Date.now() - p.approvedAt) < 24 * 60 * 60 * 1000 // today
                  );
                  if (!sameAmtApproved) {
                    found = true;
                    console.log(`[Gmail] ✅ MATCHED BY AMT+KW (small amt, no duplicate): ₹${amt}`);
                  } else {
                    console.log(`[Gmail] ⚠️ SKIPPED: Same amount ₹${amt} already approved today — need refId match`);
                  }
                }
                // For amounts > ₹10, require refId match to prevent false positives
                else if (amtOk && kwOk && amt > 10) {
                  console.log(`[Gmail] ⚠️ amt+kw matched but no refId (₹${amt} > ₹10) — SKIPPING to prevent false match`);
                }
              } catch(pe) { console.warn('[Gmail] parse err:', pe.message); }
            });
          });

          f.once('error', (fe) => { console.error('[Gmail] fetch err:', fe.message); });
          f.once('end', () => {
            imap.end();
            resolve(found
              ? { ok:true, isPaid:true,  status:'success', byRef: foundByRef }
              : { ok:true, isPaid:false, status:'pending' }
            );
          });
        });
      });
...
//  BOT INIT
// ──────────────────────────────────────────────
const bot = new TelegramBot(DEFAULTS.TOKEN, { polling: false });

// Webhook delete karke clean polling — duplicate message fix
(async () => {
  try { await bot.deleteWebHook(); console.log('[BOT] Webhook cleared.'); }
  catch(e) { console.log('[BOT] deleteWebhook:', e.message); }
  bot.startPolling({ interval: 1000, params: { timeout: 30, allowed_updates: ['message','callback_query'] } });
...
const CANCEL = { reply_markup:{ keyboard:[['❌ 𝐂𝐚𝐧𝐜𝐞𝐥']], resize_keyboard:true }};
...
bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
  ...
  // ── Welcome message — custom emoji forward support ──
  const wFwd = db.settings.welcomeFwd;
  if (wFwd && wFwd.fromChatId && wFwd.messageId) {
    try {
      await bot.copyMessage(uid, wFwd.fromChatId, wFwd.messageId, { reply_markup: MAIN.reply_markup });
    } catch(e) {
      console.log('[Welcome] copyMessage failed:', e.message);
      const wMsg = db.settings.welcomeMsg || '🚀 <b>Boost Your Social Media with Us!</b>\n━━━━━━━━━━━━━━━━━━━━\n🛒 <b>Order Services</b> - High-quality SMM services.\n💰 <b>Add Funds</b> - Easy UPI recharges.\n📦 <b>Track Orders</b> - Real-time order status.\n🔗 <b>Refer &amp; Earn</b> - Get amazing commissions!\n🎧 <b>24/7 Support</b> - Always here to help.';
...
bot.on('message', async (msg) => {
  ...
  if (uid === DEFAULTS.ADMIN) {
    if (text==='🔙 𝐌𝐚𝐢𝐧 𝐌𝐞𝐧𝐮')    { clearState(uid); return send(uid,'🏠 Main Menu',MAIN); }
  ...
  if (isStaff(db, uid)) {
    if (text==='🔙 𝐌𝐚𝐢𝐧 𝐌𝐞𝐧𝐮')    { clearState(uid); return send(uid,'👨\u200d💼 Staff Panel',STAFF_KB); }
  ...
  if (text==='❌ 𝐂𝐚𝐧𝐜𝐞𝐥' || text==='❌ Cancel') { clearState(uid); return send(uid,'❌ 𝐂𝐚𝐧𝐜𝐞𝐥ʟᴇᴅ.',MAIN); }
  ...
  switch(text) {
    case '💰 𝐖𝐚𝐥𝐥𝐞𝐭':    return walletMenu(uid,db,user,C);
    case '🛒 𝐍𝐞𝐰 𝐎𝐫𝐝𝐞𝐫': return newOrder(uid,db,C);
    case '📦 𝐌𝐲 𝐎𝐫𝐝𝐞𝐫𝐬': return myOrders(uid,db,user,C);
    case '🔗 𝐑𝐞𝐟𝐞𝐫𝐫𝐚𝐥':  return referralMenu(uid,db,user,C);
    case '🎧 𝐒𝐮𝐩𝐩𝐨𝐫𝐭':   return supportMenu(uid);
    case '📊 𝐌𝐲 𝐒𝐭𝐚𝐭𝐬':  return myStats(uid,db,user,C);
    case 'ℹ️ 𝐇𝐞𝐥𝐩':      return helpMenu(uid,C);
...
async function handleMedia(msg) {
  ...
  // ── WELCOME FWD — admin ne custom emoji wala message bheja ──
  if (state.step === 'a_welcome_fwd' && uid === DEFAULTS.ADMIN) {
    const db2=loadDB();
    db2.settings.welcomeFwd = {
      fromChatId: msg.chat.id,
      messageId: msg.message_id,
    };
    db2.settings.welcomeMsg='';
    saveDB(db2); clearState(uid);
    return send(uid,
      '✅ <b>Custom Emoji Welcome Set!</b>\n\nAb jab bhi koi /start karega, yahi message milega with your custom emoji! 🎉\n\n💡 Change karne ke liye: ⚙️ Settings → 💬 Welcome Msg',
      ADMIN_KB
    );
  }
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='payment_amount_gw') {
    ...
    // ✅ Local ref_id generate karo — koi bahar ka gateway nahi
    const ref_id = genRefId();

    const expiresAt = Date.now() + 30 * 60 * 1000; // 30 min
    const db2 = loadDB(); const user2 = getUser(db2, uid);
    db2.payments[ref_id] = {
      id: ref_id, userId: String(uid), amount: amt,
      gwOrderId: ref_id, txnId: ref_id,
      status: 'pending', type: 'gateway',
      createdAt: Date.now(), expiresAt,
    };
    user2.paymentHistory.push(ref_id);
    log(db2, { type: 'gateway_payment_created', userId: uid, paymentId: ref_id, amount: amt });
    saveDB(db2); clearState(uid);

    const qrImageUrl = gatewayQrImageUrl(amt, ref_id);
    const expireStr = new Date(expiresAt).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'});
    const caption =
        `💳 <b>Complete Your Payment</b>\n\n` +
        `━━━━━━━━━━━━━━━━━━━━\n` +
        `💰 Amount: <b>₹${amt}</b>\n` +
        `🆔 Ref ID: <code>${ref_id}</code>\n` +
        `🏦 UPI ID: <code>${C.upiId}</code>\n` +
        `⏰ Expires: <b>${expireStr}</b> (30 minutes)\n` +
        `━━━━━━━━━━━━━━━━━━━━\n` +
        `📲 Scan QR → Pay → Check Status below`;
    const replyKb = { inline_keyboard: [
        [{ text: '✅ 𝐂𝐡𝐞𝐜𝐤 𝐏𝐚𝐲𝐦𝐞𝐧𝐭 𝐒𝐭𝐚𝐭𝐮𝐬', callback_data: `gw_check_${ref_id}` }],
        [{ text: '🔄 𝐍𝐞𝐰 𝐐𝐑', callback_data: `gw_newqr_${ref_id}` }],
    ]};
    try {
      await bot.sendPhoto(uid, qrImageUrl, { caption, parse_mode: 'HTML', reply_markup: replyKb });
    } catch(e) {
      console.error('[QR send error]', e.message);
      // QR image fail hogi toh plain text bhejo
      await send(uid, caption + `\n\n🔗 <a href="${qrImageUrl}">QR Link</a>`, { parse_mode:'HTML', reply_markup: replyKb });
    }
    return;
  }
    ...
    try {
      await bot.sendPhoto(uid, upiQR(amt,C.upiId,C.upiName), {
        caption:
          ...
          `👤 Name: ${C.upiName}\n` +
          `━━━━━━━━━━━━━━━━━━━━\n` +
          `After payment, please send your <b>UTR Number</b> 👇`,
  ...
  if (state.step==='order_link') {
    const linkType = state.data.linkType || 'DEFAULT';
    const result = validateLink(text, linkType);
    if (!result.ok) return send(uid, result.msg, CANCEL);
    setState(uid,{step:'order_qty',data:{...state.data,link:result.link}});
    const rate=state.data.rate;
    return send(uid,
      `✅ <b>Link accepted!</b>\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `📊 Min: <b>${state.data.min}</b>  |  Max: <b>${state.data.max}</b>\n` +
      `💵 Rate: ₹${rate}/1K\n\n` +
      `🔢 Enter quantity:`,
      CANCEL
    );
  ...
  if (state.step==='order_confirm') {
    if (text==='✅ 𝐂𝐨𝐧𝐟𝐢𝐫𝐦 𝐎𝐫𝐝𝐞𝐫') return placeOrderFinal(uid,state.data,C);
  ...
  // User: service search
  if (state.step==='search_svc') {
    clearState(uid);
    const db2=loadDB(), C2=cfg(db2);
    const kw=text.toLowerCase().trim();
    if (!kw || kw.length<2) return send(uid,'⚠️ Please type at least 2 characters to search.',CANCEL);
    await send(uid,'🔍 Searching...');
    const services=await getAllServices(db2,C2);
    const results=services.filter(s=>
      s.name?.toLowerCase().includes(kw) ||
      s.category?.toLowerCase().includes(kw)
    ).slice(0,15);
    if (!results.length) return send(uid,`❌ No services found for "<b>${text}</b>".\n\nTry different keywords.`,
      {reply_markup:{inline_keyboard:[[{text:'🔙 𝐁𝐚𝐜𝐤 𝐭𝐨 𝐂𝐚𝐭𝐞𝐠𝐨𝐫𝐢𝐞𝐬',callback_data:'new_order_retry'}]]}}
    );
    // Build quick result buttons
    const cats2={};
    for (const s of services) { const c=s.category||'Other'; if(!cats2[c]) cats2[c]=[]; cats2[c].push(s); }
    const catKeys2=Object.keys(cats2).sort((a,b)=>a.localeCompare(b));
    const catMap2={}; catKeys2.forEach((c,i)=>catMap2[i]=c);
    const catData2={}; catKeys2.forEach((c,i)=>{ catData2[i]=cats2[c]; });
    const svcMap2={};
    results.forEach((s,i)=>svcMap2[i]=s);
    setState(uid,{catMap:catMap2,catData:catData2,services,svcMap:svcMap2});
    const btns=results.map((s,i)=>[{text:`${safeSlice(s.name,45)}`,callback_data:`osvc_${i}`}]);
    btns.push([{text:'🔙 𝐁𝐚𝐜𝐤 𝐭𝐨 𝐂𝐚𝐭𝐞𝐠𝐨𝐫𝐢𝐞𝐬',callback_data:'new_order_retry'}]);
    return send(uid,
      `🔍 <b>Search: "${safeSlice(text,30)}"</b>\n\n${results.length} result(s) found:`,
  ...
  if (state.step==='a_broadcast') {
    ...
    // Custom emoji preserve karne ke liye — entities extract karo admin ke message se
    const msgEntities = msg.entities || [];
    // Broadcast header text length — "📢 Announcement\n\n" = 18 chars
    const HEADER = '📢 <b>Announcement</b>\n\n';
    const HEADER_LEN = '📢 Announcement\n\n'.length; // plain length for offset calc
    // Shift entities by header length
    const shiftedEntities = msgEntities.map(e => ({ ...e, offset: e.offset + HEADER_LEN }));
    // Add bold entity for "Announcement" header
    const announcementEntity = { type:'bold', offset:2, length:12 }; // "Announcement"
    const allEntities = [announcementEntity, ...shiftedEntities];
    for (const id of uids) {
      try {
        // Try with entities first (preserves custom emoji)
        await bot.sendMessage(id, `📢 Announcement\n\n${text}`, {
          entities: allEntities,
        });
        ok++;
      } catch {
        // Fallback: HTML parse mode (no custom emoji but still works)
        try {
          await send(id, `📢 <b>Announcement</b>\n\n${text}`);
          ok++;
        } catch { fail++; }
      }
      await sleep(50);
  ...
  if (state.step==='a_set_apiurl') {
    ...
    const db2=loadDB(); db2.settings.apiUrl=text;
    // Clear ALL old pinned API services — old API ke services naye API par kaam nahi karte
    const oldPinnedCount = Object.keys(db2.apiServices||{}).length;
    db2.apiServices = {};
    saveDB(db2);
    // Clear service cache so users see fresh services from new API
    _svcCache = null; _svcCacheTs = 0;
    clearState(uid);
    return send(uid,
      `✅ <b>API URL saved!</b>\n<code>${text}</code>\n\n` +
      `🗑 <b>${oldPinnedCount} old pinned services deleted</b> (purani API ke services remove ho gaye)\n` +
      `🔄 Service cache cleared\n\n` +
      `💡 Ab 🛠 Services Mgmt → 🌐 Browse & Add API Services se naye services add karein.`,
      ADMIN_KB
    );
  }
  if (state.step==='a_set_apikey') {
    const db2=loadDB(); db2.settings.apiKey=text;
    // Clear ALL old pinned API services — old API key ke services naye key par kaam nahi karte
    const oldPinnedCount = Object.keys(db2.apiServices||{}).length;
    db2.apiServices = {};
    saveDB(db2);
    // Clear service cache so users see fresh services from new API key
    _svcCache = null; _svcCacheTs = 0;
    clearState(uid);
    return send(uid,
  ...
  if (state.step==='a_set_margin') {
    ...
    const db2=loadDB(); db2.settings.margin=m; saveDB(db2);
    // No need to clear svcCache — rates are computed live via addMargin() at display time
    clearState(uid);
    return send(uid,`✅ Margin set to: ${m}%\n\n💡 New prices are effective immediately for all users.`,ADMIN_KB);
  }
  ...
  if (state.step==='a_welcome_msg') {
    const db2=loadDB();
    db2.settings.welcomeMsg=text;
    db2.settings.welcomeFwd=null; // text set kiya to forward clear karo
    saveDB(db2); clearState(uid);
    return send(uid,'✅ Welcome message updated!\n\n👀 Preview:\n\n'+text,ADMIN_KB);
  }
  if (state.step==='a_welcome_fwd') {
    // User ne custom emoji wala message bheja — iska chat_id aur message_id save karo
    const db2=loadDB();
    db2.settings.welcomeFwd = {
      fromChatId: msg.chat.id,
      messageId: msg.message_id,
    };
    db2.settings.welcomeMsg=''; // text wala clear karo
    saveDB(db2); clearState(uid);
    return send(uid,
      '✅ <b>Custom Emoji Welcome Set!</b>\n\nAb jab bhi koi /start karega, yahi message milega with your custom emoji! 🎉\n\n💡 Change karne ke liye: ⚙️ Settings → 💬 Welcome Msg',
      ADMIN_KB
  ...
  if (state.step==='as_add_id') {
    ...
    return send(uid,
      ...
      {reply_markup:{keyboard:[['✅ 𝐂𝐨𝐧𝐟𝐢𝐫𝐦 𝐀𝐝𝐝','❌ 𝐂𝐚𝐧𝐜𝐞𝐥']],resize_keyboard:true}}
    );
  }

  if (state.step==='as_confirm') {
    if (text!=='✅ 𝐂𝐨𝐧𝐟𝐢𝐫𝐦 𝐀𝐝𝐝') { clearState(uid); return send(uid,'❌ 𝐂𝐚𝐧𝐜𝐞𝐥ʟᴇᴅ.',ADMIN_KB); }
    const db2=loadDB();
...
async function walletMenu(uid,db,user,C) {
  await send(uid,
    ...
    {reply_markup:{inline_keyboard:[
      [{text:'⚡ 𝐔𝐏𝐈 (𝐀𝐮𝐭𝐨)',    callback_data:'wallet_add'}],
      [{text:'🏦 𝐔𝐏𝐈 (𝐌𝐚𝐧𝐮𝐚𝐥)', callback_data:'wallet_manual'}],
      [{text:'₿  𝐂𝐫𝐲𝐩𝐭𝐨',       callback_data:'wallet_crypto'}],
      [{text:'📋 𝐏𝐚𝐲𝐦𝐞𝐧𝐭 𝐇𝐢𝐬𝐭𝐨𝐫𝐲', callback_data:'wallet_hist'}],
    ]}}
  );
}

// ── Fixed top platform names (case-insensitive match against category names) ──
const FIXED_PLATFORMS = [
  { label: 'Telegram',  match: /telegram/i  },
  { label: 'Instagram', match: /instagram/i },
  { label: 'WhatsApp',  match: /whatsapp/i  },
  { label: 'YouTube',   match: /youtube/i   },
  { label: 'Snapchat',  match: /snapchat/i  },
  { label: 'TikTok',    match: /tiktok/i    },
  { label: 'Twitter',   match: /twitter|x\.com/i },
  { label: 'Facebook',  match: /facebook/i  },
];
const PAGE_SIZE_CAT = 8; // categories per page (Telegram button limit safe)

async function newOrder(uid, db, C, page = 0) {
  try {
    const loadMsg = await bot.sendMessage(uid, '⏳ <b>Loading services...</b>', { parse_mode: 'HTML' });
    const services = await getAllServices(db, C);
    try { await bot.deleteMessage(uid, loadMsg.message_id); } catch (_) {}

    if (!services.length) {
      // API se check karo reason
      const testRes = await smmAPI(C.apiUrl, C.apiKey, { action:'services' });
      let errMsg = '❌ <b>Services load nahi hui!</b>\n\n';
      if (!testRes.ok) {
        if (testRes.error && testRes.error.includes('403'))
          errMsg += '🔑 <b>Reason:</b> Invalid API Key\n\n💡 Admin se contact karo.';
        else if (testRes.error && testRes.error.includes('404'))
          errMsg += '🌐 <b>Reason:</b> Wrong API URL\n\n💡 Admin se contact karo.';
        else if (testRes.error && testRes.error.includes('timeout'))
          errMsg += '⏱ <b>Reason:</b> API server slow hai\n\n💡 2-3 minute wait karke retry karo.';
        else
          errMsg += `⚠️ <b>Error:</b> <code>${testRes.error || 'Unknown'}</code>\n\n💡 Thodi der baad retry karo.`;
      } else {
        errMsg += '⚠️ API ne empty list return ki.\n\n💡 Thodi der baad retry karo.';
      }
      return send(uid, errMsg,
        { reply_markup: { inline_keyboard: [
          [{ text: '🔄 Retry', callback_data: 'new_order_retry' }, { text: '🏠 𝐇𝐨𝐦𝐞', callback_data: 'go_home' }]
        ]}}
      );
    }
    ...
    // All category keys sorted A→Z
    const catKeys = Object.keys(cats).sort((a, b) => a.localeCompare(b));
    ...
    btns.push([{ text: '🔍 𝐒𝐞𝐚𝐫𝐜𝐡 𝐂𝐚𝐭𝐞𝐠𝐨𝐫𝐢𝐞𝐬 & 𝐒𝐞𝐫𝐯𝐢𝐜𝐞𝐬', callback_data: 'search_services' }]);

    // ── Rows 2-3: Fixed platform buttons (3 per row, then 2) ──
    const fixedBtns = [];
    for (const plat of FIXED_PLATFORMS) {
      // Find ALL matching categories for this platform
      const matchingIdxs = catKeys.map((c, i) => plat.match.test(c) ? i : -1).filter(i => i !== -1);
      if (matchingIdxs.length === 0) continue;
      if (matchingIdxs.length === 1) {
        // Only one category — go directly to it
        fixedBtns.push({ text: plat.label, callback_data: `ocat_${matchingIdxs[0]}` });
      } else {
        // Multiple categories — show sub-menu with all of them
        fixedBtns.push({ text: plat.label, callback_data: `oplatform_${plat.label.toLowerCase()}` });
      }
    }
    // Split fixed buttons into rows of 3
    for (let r = 0; r < fixedBtns.length; r += 3) {
      btns.push(fixedBtns.slice(r, r + 3));
    }

    // ── Alphabetical list for current page ──
    const totalPages   = Math.ceil(catKeys.length / PAGE_SIZE_CAT);
    const pageCatKeys  = catKeys.slice(page * PAGE_SIZE_CAT, (page + 1) * PAGE_SIZE_CAT);

    for (const c of pageCatKeys) {
      const i = catKeys.indexOf(c);
      btns.push([{ text: c, callback_data: `ocat_${i}` }]);
    }

    // ── Pagination row ──
    const navRow = [];
    if (page > 0)                  navRow.push({ text: '⬅️ 𝐏𝐫𝐞𝐯', callback_data: `ocat_page_${page - 1}` });
    if (page < totalPages - 1)     navRow.push({ text: `𝐍𝐞𝐱𝐭 ▶️`, callback_data: `ocat_page_${page + 1}` });
    if (totalPages > 1)            navRow.push({ text: `📄 ${page + 1}/${totalPages}`, callback_data: 'noop' });
    if (navRow.length)             btns.push(navRow);

    // ── Bottom nav ──
    btns.push([
      { text: '📦 𝐌𝐲 𝐎𝐫𝐝𝐞𝐫𝐬', callback_data: 'my_orders' },
      { text: '🏠 𝐇𝐨𝐦𝐞',      callback_data: 'go_home'   },
    ]);

    // Direct bot.sendMessage — inline_keyboard ko send() wrapper se bypass karo
    try {
      await bot.sendMessage(uid,
        `🛒 Categories\n\nSorted A → Z · Page ${page + 1}\nSelect a category:`,
        { reply_markup: { inline_keyboard: btns } }
      );
    } catch(sendErr) {
      console.error('[newOrder sendMessage error]', sendErr.message);
      await bot.sendMessage(uid,
    ...
    return send(uid,
      ...
      { reply_markup: { inline_keyboard: [
        [{ text: '🔄 Try Again', callback_data: 'new_order_retry' }, { text: '🏠 𝐇𝐨𝐦𝐞', callback_data: 'go_home' }]
      ]}}
    );
  }
}
...
` +
  ...
  const btns = {reply_markup:{inline_keyboard:[
    [{text:'💰 𝐀𝐝𝐝 𝐅𝐮𝐧𝐝𝐬',callback_data:'wallet_add'},{text:'📦 𝐌𝐲 𝐎𝐫𝐝𝐞𝐫𝐬',callback_data:'orders_refresh'}],
    [{text:'🔄 𝐑𝐞𝐟𝐫𝐞𝐬𝐡 𝐒𝐭𝐚𝐭𝐬',callback_data:'mystats_refresh'}],
  ]}};
...
async function myOrders(uid,db,user,C) {
  ...
  for (const oid of recent) {
    ...
    txt+=`🆔 <code>${displayOrderId(o)}</code>${o.isCustom?' 🔧':' 🌐'}\n📌 ${safeSlice(o.serviceName,40)}\n🔢 ${o.quantity} | ₹${o.cost} | ${statusEmoji(o.status)} ${o.status}\n⏱ ${timeAgo(o.createdAt)}\n\n`;

    // 🔄 Refill button — ONLY if:
    // 1. Not a custom order (must be API order)
    // 2. Service was marked refillable at order time
    // 3. Order is completed or partial (only these states qualify for refill)
    if (!o.isCustom && o.refillable && ['completed','partial'].includes(st)) {
      if (o.refillId) {
        // Already requested — show status button
        btnRows.push([{text:`🔁 Refill Status — ${o.id}`,callback_data:`refill_st_${o.id}`}]);
      } else {
        // Not yet requested — show request button
        btnRows.push([{text:`🔄 Request Refill — ${o.id}`,callback_data:`refill_req_${o.id}`}]);
      }
    }
  }

  btnRows.push([{text:'🔄 𝐑𝐞𝐟𝐫𝐞𝐬𝐡 𝐒𝐭𝐚𝐭𝐮𝐬',callback_data:'orders_refresh'}]);
  await send(uid,txt,{reply_markup:{inline_keyboard:btnRows}});
}
...
async function helpMenu(uid,C) {
  await send(uid,
    ...
    `💰 <b>How to Recharge?</b>\n` +
    `Wallet → Add Funds → Pay via UPI → Submit UTR + Screenshot\n\n` +
    `🔗 <b>Referral Program</b>\n` +
    `Share your link → Earn ${C.refPct}% commission on every recharge\n\n` +
    `🎧 <b>Need Help?</b>\n` +
    `Tap Support and describe your issue\n\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
...
async function confirmOrder(uid,data,qty,db,C) {
  ...
  await send(uid,
    ...
    {reply_markup:{keyboard:[['✅ 𝐂𝐨𝐧𝐟𝐢𝐫𝐦 𝐎𝐫𝐝𝐞𝐫'],['❌ 𝐂𝐚𝐧𝐜𝐞𝐥']],resize_keyboard:true}}
  );
}
...
async function placeOrderFinal(uid,data,C) {
  ...
  if (!custom) {
    ...
    // Log the FULL raw response so we can see exactly what the panel sent back
    console.log('[ORDER] Raw panel response:', JSON.stringify(res.data));
    const errFromBody = res.data?.error || res.data?.message || res.data?.msg ||
      (typeof res.data==='string' ? res.data : null);
    if (!res.ok || errFromBody) {
      const err = errFromBody || res.error || 'Unknown error';
      console.error('[ORDER FAIL]', err);
      return send(uid,`❌ <b>Order failed!</b>\n\nError: <code>${err}</code>\n\n💡 Please contact Support.`,MAIN);
    }
    // Try every common field name panels use for the order id.
    const rawId = res.data?.order ?? res.data?.id ?? res.data?.orderId ??
      res.data?.orderid ?? res.data?.order_id ?? null;
    if (rawId === null || rawId === undefined || rawId === '') {
      // Panel didn't return a usable order ID — DO NOT fake one.
      // Faking it here is what causes "refill fail" / status-check failures later,
      // because the bot would query the panel with an ID it never issued.
      console.error('[ORDER FAIL] No order ID in panel response:', JSON.stringify(res.data));
      return send(uid,
        `❌ <b>Order failed!</b>\n\n` +
        `The panel did not return an order ID, so the order was NOT placed.\n` +
        `Raw response: <code>${safeSlice(JSON.stringify(res.data),200)}</code>\n\n` +
        `💡 Your balance was not deducted. Please contact Support / check API URL & Key.`,
        MAIN
      );
    }
    apiId=String(rawId);
  ...
  // Check if service supports refill — saved at order time from API services list
  let refillable = false;
  if (!custom) {
    try {
      const svcList = await getAllServices(db2, C);
      const svcInfo = svcList.find(s => String(s.service) === String(data.serviceId));
      if (svcInfo) refillable = !!(svcInfo.refill);
    } catch(e) { console.log('[ORDER] refillable check failed:', e.message); }
  }
  ...
  db2.orders[oid]={
    ...
    isCustom:!!custom,
    refillable,          // ← true only if API service has refill:true
    refillId:null,       // ← set when user requests refill
    refillRequestedAt:null,
    linkType:data.linkType||'DEFAULT', // ← type of link used (TELEGRAM_INVITE, INSTAGRAM_POST, etc.)
  ...
  await send(uid,
    ...
    `${custom ? '' : `🔖 Ref: <code>${oid}</code>\n`}` +
    `📌 ${data.svcName}\n` +
    `🔗 <code>${data.link}</code>\n` +
    `🔢 Quantity: <b>${data.qty}</b>\n` +
    `💰 Amount: <b>₹${data.cost}</b>\n` +
    `💳 Balance: ₹${user.balance.toFixed(2)}\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `${statusEmoji('pending')} Status: <b>Processing</b>\n` +
    `${refillable ? '🔄 Refill supported — available after completion\n' : ''}` +
    `📲 You will be notified when your order is delivered.`,
    MAIN
  );
  // Admin notification
...
async function showASBServicePage(uid, db, C, svcs, catName, catIdx, page, msgId) {
  ...
  // Pagination nav
  const navRow = [];
  if (page > 0) navRow.push({text:`⬅️ 𝐏𝐫𝐞𝐯`, callback_data:`asb_svcp_${page-1}`});
  ...
  btns.push([{text:'🔙 𝐁𝐚𝐜𝐤 𝐭𝐨 𝐂𝐚𝐭𝐞𝐠𝐨𝐫𝐢𝐞𝐬', callback_data:`asb_start`}]);
...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('gw_check_')) {
    ...
    if (pay.expiresAt && Date.now() > pay.expiresAt) {
      ...
      return send(uid,
        ...
        {reply_markup:{inline_keyboard:[
          ...
          [{text:'🎧 ᴄᴏɴᴛᴀᴄᴛ 𝐒𝐮𝐩𝐩𝐨𝐫𝐭', callback_data:'open_support'}],
        ]}}
      );
    }
    ...
    // ── Gmail/IMAP connection error — don't show "Failed", show retry ──
    if (!chk.ok) {
      return send(uid,
        `⚠️ <b>Verification Temporarily Unavailable</b>\n\n` +
        `💰 Amount: ₹${pay.amount}\n` +
        `🆔 Order ID: <code>${pid}</code>\n\n` +
        `Our payment verification system is busy. Please wait 1-2 minutes and try again.\n\n` +
        `If you've already paid, tap below to check again.`,
        {reply_markup:{inline_keyboard:[
          [{text:'🔄 𝐂𝐡𝐞𝐜𝐤 𝐀𝐠𝐚𝐢𝐧', callback_data:`gw_check_${pid}`}],
          [{text:'🎧 ᴄᴏɴᴛᴀᴄᴛ 𝐒𝐮𝐩𝐩𝐨𝐫𝐭', callback_data:'open_support'}],
        ]}}
      );
    }
    ...
    // ── No payment received yet — show pending with retry ──
    // Track retry count
    pay.checkCount = (pay.checkCount || 0) + 1;
    saveDB(db);

    if (pay.checkCount <= 4) {
      // Pehle 4 attempts — pending dikhao, retry karne do
      return send(uid,
        `⏳ <b>Payment Pending</b>\n\n` +
        `💰 Amount: ₹${pay.amount}\n` +
        `🆔 Order ID: <code>${pid}</code>\n\n` +
        `🔍 Payment not detected yet.\n` +
        `• Agar abhi pay kiya hai toh 1-2 min wait karo\n` +
        `• Phir "Check Again" dabao\n\n` +
        `<i>Attempt ${pay.checkCount}/4</i>`,
        {reply_markup:{inline_keyboard:[
          [{text:'🔄 𝐂𝐡𝐞𝐜𝐤 𝐀𝐠𝐚𝐢𝐧', callback_data:`gw_check_${pid}`}],
          [{text:'🔄 𝐍𝐞𝐰 𝐐𝐑', callback_data:`gw_newqr_${pid}`}],
          [{text:'🎧 𝐂𝐨𝐧𝐭𝐚𝐜𝐭 𝐒𝐮𝐩𝐩𝐨𝐫𝐭', callback_data:'open_support'}],
        ]}}
      );
    }

    // 4+ attempts ke baad — failed dikhao
    try { await bot.deleteMessage(uid, q.message.message_id); } catch{}
    return send(uid,
      `❌ <b>Payment Not Found!</b>\n\n` +
      `💰 Amount: ₹${pay.amount}\n` +
      `🆔 Order ID: <code>${pid}</code>\n\n` +
      `⚠️ Kai baar check karne ke baad bhi payment nahi mila.\n\n` +
      `• Agar payment ho gayi hai toh Support se contact karo\n` +
      `• Naya recharge karne ke liye "Try Again" dabao`,
      {reply_markup:{inline_keyboard:[
        [{text:'💰 𝐓𝐫𝐲 𝐀𝐠𝐚𝐢𝐧', callback_data:'wallet_add'}],
        [{text:'🎧 𝐂𝐨𝐧𝐭𝐚𝐜𝐭 𝐒𝐮𝐩𝐩𝐨𝐫𝐭', callback_data:'open_support'}],
      ]}}
    );
  }
  ...
  if (data.startsWith('gw_newqr_') ) {
    ...
    // ✅ Naya ref_id locally generate karo
    let newPid;
    try {
      // Local ref_id — koi bahar ka gateway nahi
      newPid = genRefId();
    } catch (e) {
      saveDB(db2);
      return send(uid, `❌ Error: ${e.message}. Thodi der baad try karo.`);
    }
    ...
    try {
      await bot.sendPhoto(uid, qrImageUrl, {
        caption:
          ...
          `🆔 Ref ID: <code>${newPid}</code>\n` +
          `🏦 UPI ID: <code>${Cfg.upiId}</code>\n` +
          `⏰ Expires: <b>${expireStr}</b> (30 minutes)\n` +
          `━━━━━━━━━━━━━━━━━━━━\n` +
        ...
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [
          [{ text: '✅ 𝐂𝐡𝐞𝐜𝐤 𝐏𝐚𝐲𝐦𝐞𝐧𝐭 𝐒𝐭𝐚𝐭𝐮𝐬', callback_data: `gw_check_${newPid}` }],
        ]},
      });
    } catch {
      await send(uid,
        `🆕 New QR Code:\n💰 ₹${pay.amount}\n🆔 <code>${newPid}</code>\n⏰ Valid until ${expireStr}`,
        {reply_markup:{inline_keyboard:[[{text:'✅ 𝐂𝐡𝐞𝐜𝐤 𝐏𝐚𝐲𝐦𝐞𝐧𝐭 𝐒𝐭𝐚𝐭𝐮𝐬',callback_data:`gw_check_${newPid}`}]]}}
      );
    }
    return;
  }
  ...
  if (data==='wallet_crypto')  {
    ...
    return send(uid,
      ...
      {reply_markup:{inline_keyboard:[
        ...
        [{text:'🔙 𝐁𝐚𝐜𝐤', callback_data:'wallet_back'}],
      ]}}
    );
  }
  ...
  if (data==='mystats_refresh') {
    return myStats(uid,db,getUser(db,uid),C);
  }

  if (data==='orders_refresh') {
    const u=getUser(db,uid);
    // Refresh all active (not custom) orders including partial
    const apiOrds=u.orderHistory.map(oid=>db.orders[oid]).filter(o=>o&&!o.isCustom&&['pending','inprogress','processing','partial'].includes((o.status||'').toLowerCase()));
    if (apiOrds.length) {
      ...
      if (res.ok&&typeof res.data==='object') {
        for (const o of apiOrds) {
          const s=res.data[o.apiOrderId];
          if(s?.status){ o.status=s.status.toLowerCase(); o.updatedAt=Date.now(); }
        }
        saveDB(db);
      }
    }
  ...
  if (data==='new_order_retry') {
    ...
    try { await bot.deleteMessage(uid, q.message.message_id); } catch(_){}
    const st=getState(uid);
    return newOrder(uid,db,C, st.orderPage||0);
  }
  if (data==='go_home') { clearState(uid); return send(uid,'🏠 Main Menu',MAIN); }
  if (data==='noop') return; // page indicator button, do nothing
  if (data==='my_orders') { const u=getUser(db,uid); return myOrders(uid,db,u,C); }
  if (data==='search_services') {
    setState(uid,{...getState(uid), step:'search_svc'});
    return send(uid,'🔍 <b>Search Services</b>\n\nType a keyword to search (e.g. "Instagram followers", "Telegram members"):',CANCEL);
  }

  // ── PLATFORM SUB-MENU: show all matching categories for a platform (e.g. all Telegram cats) ──
  if (data.startsWith('oplatform_')) {
    const platLabel = data.slice(10); // e.g. 'telegram'
    const plat = FIXED_PLATFORMS.find(p => p.label.toLowerCase() === platLabel);
    if (!plat) return newOrder(uid, db, C);
    try {
      const st = getState(uid);
      let catMap = st.catMap;
      let catData = st.catData;
      // If state is empty, re-fetch
      if (!catMap || !catData) {
        await send(uid, '⏳ Loading...');
        ...
        const catKeysArr = Object.keys(catsObj).sort((a, b) => a.localeCompare(b));
        catMap = {}; catKeysArr.forEach((c, i) => catMap[i] = c);
        catData = {}; catKeysArr.forEach((c, i) => { catData[i] = catsObj[c]; });
        setState(uid, { ...st, catMap, catData });
      }
      // Find all categories matching this platform
      const matchingEntries = Object.entries(catMap).filter(([, name]) => plat.match.test(name));
      if (!matchingEntries.length) return newOrder(uid, db, C);
      try { await bot.deleteMessage(uid, q.message.message_id); } catch (_) {}
      const btns = matchingEntries.map(([idx, name]) => [{
        text: `${name} (${(catData[idx] || []).length} services)`,
        callback_data: `ocat_${idx}`
      }]);
      btns.push([{ text: '🔙 𝐁𝐚𝐜𝐤 𝐭𝐨 𝐂𝐚𝐭𝐞𝐠𝐨𝐫𝐢𝐞𝐬', callback_data: 'ocat_back' }]);
      return send(uid,
        `📁 <b>${plat.label} Services</b>\n\n🔢 ${matchingEntries.length} categories available:\nSelect a category:`,
        { reply_markup: { inline_keyboard: btns } }
      );
    } catch (e) {
      console.error('[oplatform_ error]', e.message);
      return newOrder(uid, db, C);
    }
  }

  if (data.startsWith('ocat_page_')) {
    let pg = parseInt(data.slice(10), 10) || 0;
    // Clamp page to valid range using services from state
    const st0 = getState(uid);
    if (st0.catMap) {
      const totalCats = Object.keys(st0.catMap).length;
      const maxPage = Math.max(0, Math.ceil(totalCats / PAGE_SIZE_CAT) - 1);
      pg = Math.max(0, Math.min(pg, maxPage));
    }
    try { await bot.deleteMessage(uid, q.message.message_id); } catch(_){}
    return newOrder(uid,db,C,pg);
  }
  ...
  if (data.startsWith('ocat_')) {
    ...
    try {
      ...
      // First try: use STATE (fast, no API call) — NO slice limit, full list
      if (st.catMap && st.catMap[idx] && st.catData && st.catData[idx] && st.catData[idx].length) {
        catName=st.catMap[idx];
        svcsForCat=st.catData[idx]; // ✅ FIXED: removed .slice(0,20) — show ALL services
      }
      ...
      if (!svcsForCat || !svcsForCat.length) {
        ...
        const catsObj={};
        for (const s of allSvcs) { const c=s.category||'Other'; if(!catsObj[c]) catsObj[c]=[]; catsObj[c].push(s); }
        const catKeysArr=Object.keys(catsObj).sort((a,b)=>a.localeCompare(b)); // ✅ FIX: sorted A→Z to match button indexes
        catName=catKeysArr[idx]||null;
        if (!catName) return newOrder(uid,db,C);
        svcsForCat=(catsObj[catName]||[]); // ✅ FIXED: removed .slice(0,20)
        const catMapR={}; catKeysArr.forEach((c,i)=>catMapR[i]=c);
        const catDataR={}; catKeysArr.forEach((c,i)=>{ catDataR[i]=catsObj[c]; });
        setState(uid,{...st, catMap:catMapR, catData:catDataR, services:allSvcs}); // ✅ FIX: save services too
      }

      if (!svcsForCat || !svcsForCat.length) return send(uid,'❌ No services found in this category. Please tap 🔙 Back and try again.',MAIN);

      // ── PAGINATED display — sab services dikhao, 10 per page ──
      const SVC_PAGE_SIZE = 10;
      const svcPageNum = 0;
      const totalSvcPages = Math.ceil(svcsForCat.length / SVC_PAGE_SIZE);
      const svcSlice = svcsForCat.slice(0, SVC_PAGE_SIZE);

      const svcMap={};
      svcsForCat.forEach((s,i)=>svcMap[i]=s);
      setState(uid,{...getState(uid), svcMap, lastCatIdx:idx, lastCatName:catName, svcPageData:svcsForCat, svcPage:svcPageNum});

      const btns=svcSlice.map((s,i)=>[{text:`${safeSlice(s.name,40)} — ₹${addMargin(s.rate,C.margin)}/1K`,callback_data:`osvc_${i}`}]);

      // Pagination nav row
      if (totalSvcPages > 1) {
        btns.push([{text:`𝐍𝐞𝐱𝐭 ▶️`, callback_data:`osvc_page_1_${idx}`}]);
      }
      btns.push([{text:'🔙 𝐁𝐚𝐜𝐤 𝐭𝐨 𝐂𝐚𝐭𝐞𝐠𝐨𝐫𝐢𝐞𝐬',callback_data:'ocat_back'}]);
      return send(uid,`📁 <b>${catName}</b>\n\n🔢 ${svcsForCat.length} services available${totalSvcPages>1?` • Page 1/${totalSvcPages}`:''}:`,{reply_markup:{inline_keyboard:btns}});
    } catch(e) {
  ...
  // ── SERVICE PAGE NAVIGATION (Next/Prev within a category) ──
  if (data.startsWith('osvc_page_')) {
    // format: osvc_page_{pageNum}_{catIdx}
    const parts = data.slice(10).split('_');
    const pageNum = parseInt(parts[0], 10) || 0;
    const catIdx  = parseInt(parts[1], 10);
    try {
      const st = getState(uid);
      const SVC_PAGE_SIZE = 10;
      let svcsForCat = st.svcPageData || null;
      let catName = st.lastCatName || (st.catMap && st.catMap[catIdx]) || null;
      ...
      if (!svcsForCat || !svcsForCat.length) {
        ...
        const catsObj = {};
        for (const s of allSvcs) { const c=s.category||'Other'; if(!catsObj[c]) catsObj[c]=[]; catsObj[c].push(s); }
        const catKeysArr = Object.keys(catsObj).sort((a,b)=>a.localeCompare(b)); // ✅ FIX: sorted
        catName = catKeysArr[catIdx] || null;
        if (!catName) return newOrder(uid, db, C);
        svcsForCat = catsObj[catName] || [];
        const svcMapR = {}; svcsForCat.forEach((s,i) => svcMapR[i]=s);
        const catMapR2={}; catKeysArr.forEach((c,i)=>catMapR2[i]=c);
        const catDataR2={}; catKeysArr.forEach((c,i)=>{catDataR2[i]=catsObj[c];});
        setState(uid, {...st, svcMap: svcMapR, svcPageData: svcsForCat, lastCatName: catName, lastCatIdx: catIdx, catMap:catMapR2, catData:catDataR2});
      }

      const totalSvcPages = Math.ceil(svcsForCat.length / SVC_PAGE_SIZE);
      const safePage = Math.max(0, Math.min(pageNum, totalSvcPages - 1));
      const startIdx = safePage * SVC_PAGE_SIZE;
      const svcSlice = svcsForCat.slice(startIdx, startIdx + SVC_PAGE_SIZE);

      setState(uid, {...getState(uid), svcPage: safePage});

      const btns = svcSlice.map((s,i) => [{
        text: `${safeSlice(s.name, 40)} — ₹${addMargin(s.rate, C.margin)}/1K`,
        callback_data: `osvc_${startIdx + i}`
      }]);

      const navRow = [];
      if (safePage > 0)                navRow.push({text:'⬅️ 𝐏𝐫𝐞𝐯', callback_data:`osvc_page_${safePage-1}_${catIdx}`});
      if (safePage < totalSvcPages-1)  navRow.push({text:'𝐍𝐞𝐱𝐭 ▶️', callback_data:`osvc_page_${safePage+1}_${catIdx}`});
      if (navRow.length) btns.push(navRow);
      btns.push([{text:'🔙 𝐁𝐚𝐜𝐤 𝐭𝐨 𝐂𝐚𝐭𝐞𝐠𝐨𝐫𝐢𝐞𝐬', callback_data:'ocat_back'}]);

      return send(uid,
        `📁 <b>${catName}</b>\n\n🔢 ${svcsForCat.length} services • Page ${safePage+1}/${totalSvcPages}:`,
        {reply_markup:{inline_keyboard:btns}}
      );
    } catch(e) {
      console.error('[osvc_page_ error]', e.message);
      return send(uid, `❌ Error loading page. Please tap 🛒 New Order and try again.\n<code>${e.message}</code>`, MAIN);
    }
  }
  ...
  if (data.startsWith('osvc_')) {
    ...
    try {
      ...
      if (!svc) {
        ...
        const catKeysOsvc = Object.keys(cats2).sort((a,b)=>a.localeCompare(b)); // ✅ FIX sorted
        const lastCat = st.lastCatName ||
          (st.lastCatIdx!=null ? catKeysOsvc[st.lastCatIdx] : null);
        if (lastCat && cats2[lastCat]) {
          const svcs2=cats2[lastCat];
          svc=svcs2[idx]||null;
          const svcMap2={};
          svcs2.forEach((s,i)=>svcMap2[i]=s);
          const catMapOsvc={}; catKeysOsvc.forEach((c,i)=>catMapOsvc[i]=c);
          const catDataOsvc={}; catKeysOsvc.forEach((c,i)=>{catDataOsvc[i]=cats2[c];});
          setState(uid,{...getState(uid), svcMap:svcMap2, svcPageData:svcs2, catMap:catMapOsvc, catData:catDataOsvc});
        }
      }
      ...
      // Detect what kind of link this service needs
      const linkType = detectLinkType(svc.name, svc.category||'');
      const linkInfo  = getLinkInfo(linkType);
      setState(uid,{...getState(uid), step:'order_link', data:{serviceId:String(svc.service),svcName:svc.name,rate,min:svc.min,max:svc.max,linkType}});
      return send(uid,
        `${linkInfo.emoji} <b>${svc.name}</b>\n` +
        `━━━━━━━━━━━━━━━━━━━━\n` +
        `💵 Rate: <b>₹${rate}/1000</b>\n` +
        `📊 Min: <b>${svc.min}</b>  |  Max: <b>${svc.max}</b>\n` +
        `${svc.delivery ? `⏱ Delivery: <b>${svc.delivery}</b>\n` : ''}` +
        `${svc.refill ? `🔄 Refill: ✅ Supported\n` : ''}` +
        `━━━━━━━━━━━━━━━━━━━━\n\n` +
        `${linkInfo.msg}`,
        CANCEL
      );
  ...
  // ── REFILL REQUEST (user side) ──
  // Only shown when service has refill:true AND order is completed/partial
  if (data.startsWith('refill_req_')) {
    const oid=data.slice(11);
    const o=db.orders[oid];
    if (!o) return send(uid,'❌ Order not found.');
    if (o.userId!==String(uid)) return send(uid,'❌ This is not your order.');
    if (o.isCustom) return send(uid,'❌ Custom orders do not support refill.');
    if (!o.refillable) return send(uid,'❌ This service does not support refill.\n\nPlease contact Support if you need help.');
    if (o.refillId) return send(uid,`⚠️ Refill already requested!\n\n🔁 Refill ID: <code>${o.refillId}</code>\n\nUse "Refill Status" button to track it.`,
      {reply_markup:{inline_keyboard:[[{text:'🔁 𝐂𝐡𝐞𝐜𝐤 𝐑𝐞𝐟𝐢𝐥𝐥 𝐒𝐭𝐚𝐭𝐮𝐬',callback_data:`refill_st_${oid}`}]]}}
    );
    const st=(o.status||'').toLowerCase();
    if (!['completed','partial'].includes(st)) return send(uid,'⚠️ Refill can only be requested for Completed or Partial orders.');

    try {
      const res=await smmAPI(C.apiUrl,C.apiKey,{action:'refill',order:o.apiOrderId});
      if (res.ok && res.data?.refill) {
        o.refillId=String(res.data.refill);
        o.refillRequestedAt=Date.now();
        saveDB(db);
        return send(uid,
          `✅ <b>Refill Requested Successfully!</b>\n\n🆔 Order: <code>${displayOrderId(o)}</code>\n📌 ${safeSlice(o.serviceName,40)}\n🔁 Refill ID: <code>${o.refillId}</code>\n\n⏳ Your refill is being processed. Use the button below to check status.`,
          {reply_markup:{inline_keyboard:[[{text:'🔁 𝐂𝐡𝐞𝐜𝐤 𝐑𝐞𝐟𝐢𝐥𝐥 𝐒𝐭𝐚𝐭𝐮𝐬',callback_data:`refill_st_${oid}`}]]}}
        );
      }
      const errMsg=res.data?.error||res.error||'Unknown error from API';
      console.error('[REFILL] API error:', errMsg);
      return send(uid,`❌ <b>Refill request failed!</b>\n\nError: <code>${errMsg}</code>\n\n💡 Please contact Support if this issue persists.`);
    } catch(e) {
      console.error('[REFILL] Exception:', e.message);
      return send(uid,`❌ Refill request failed: ${e.message}`);
    }
  }

  // ── REFILL STATUS CHECK (user side) ──
  if (data.startsWith('refill_st_')) {
    const oid=data.slice(10);
    const o=db.orders[oid];
    if (!o) return send(uid,'❌ Order not found.');
    if (o.userId!==String(uid)) return send(uid,'❌ This is not your order.');
    if (!o.refillId) return send(uid,'⚠️ No refill has been requested for this order yet.',
      {reply_markup:{inline_keyboard:[[{text:'🔄 𝐑𝐞𝐪𝐮𝐞𝐬𝐭 𝐑𝐞𝐟𝐢𝐥𝐥',callback_data:`refill_req_${oid}`}]]}}
    );
    try {
      const res=await smmAPI(C.apiUrl,C.apiKey,{action:'refill_status',refills:o.refillId});
      if (res.ok && res.data) {
        // API may return array or object
        const s=Array.isArray(res.data)?res.data[0]:res.data[o.refillId]||res.data;
        const st=s?.status||'unknown';
        const requestedAgo=o.refillRequestedAt?timeAgo(o.refillRequestedAt):'unknown';
        return send(uid,
          `🔁 <b>Refill Status</b>\n\n🆔 Order: <code>${displayOrderId(o)}</code>\n📌 ${safeSlice(o.serviceName,40)}\n🔁 Refill ID: <code>${o.refillId}</code>\n📊 Status: ${statusEmoji(st)} ${st}\n⏱ Requested: ${requestedAgo}`,
          {reply_markup:{inline_keyboard:[[{text:'🔄 𝐂𝐡𝐞𝐜𝐤 𝐀𝐠𝐚𝐢𝐧',callback_data:`refill_st_${oid}`}]]}}
        );
      }
      return send(uid,`❌ Could not fetch refill status: ${res.error||'Unknown error'}`);
    } catch(e) {
      return send(uid,`❌ Error checking refill status: ${e.message}`);
    }
  }
  ...
  if (data.startsWith('s_') && uid===DEFAULTS.ADMIN) {
    ...
    if (data==='s_welcome') {
      return send(uid,
        ...
        '2 options hain:\n\n' +
        '📝 <b>Option 1 — Text</b>\nSidha message type karo (basic emoji support)\n\n' +
        '✨ <b>Option 2 — Custom Emoji</b>\nApni chat mein custom emoji wala message bhejo → bot forward karega automatically\n\n' +
        'Kaunsa chahiye?',
        {reply_markup:{inline_keyboard:[
          [{text:'📝 Text Welcome Set Karo', callback_data:'s_welcome_text'}],
          [{text:'✨ Custom Emoji Welcome Set Karo', callback_data:'s_welcome_fwd'}],
          [{text:'🗑 Welcome Reset (Default)', callback_data:'s_welcome_reset'}],
          [{text:'❌ Cancel', callback_data:'back_settings'}],
        ]}}
      );
    }
    if (data==='s_welcome_text') { setState(uid,{step:'a_welcome_msg',data:{}}); return send(uid,'💬 Naya welcome message type karo:\n\n(HTML tags kaam karte hain: <b>bold</b>, <i>italic</i>, emoji sab)',CANCEL); }
    if (data==='s_welcome_fwd') { setState(uid,{step:'a_welcome_fwd',data:{}}); return send(uid,'✨ <b>Custom Emoji Welcome</b>\n\nAbhi apni is chat mein ek message bhejo with custom emoji — woh welcome message ban jaayega!\n\n💡 Tip: Pehle draft karo, phir yahan paste/send karo.',CANCEL); }
    if (data==='s_welcome_reset') {
      const db2=loadDB();
      db2.settings.welcomeMsg='';
      db2.settings.welcomeFwd=null;
      saveDB(db2);
      return send(uid,'✅ Welcome message reset! Ab default message dikhega.',ADMIN_KB);
    }
  ...
  if (data.startsWith('asb_catp_') && uid===DEFAULTS.ADMIN) {
    ...
    if (page > 0) navRow.push({text:`⬅️ 𝐏𝐫𝐞𝐯`, callback_data:`asb_catp_${page-1}`});
    if (page < totalPages-1) navRow.push({text:`➡️ Next`, callback_data:`asb_catp_${page+1}`});
    if (navRow.length) btns.push(navRow);
    btns.push([{text:'🔙 𝐁𝐚𝐜𝐤', callback_data:'asb_back'}]);
